...
* **深度财报**:提取标准化资产负债表/利润表/现金流量表 (`report`)，支持深度财务比率分析 (`financial`)。
* **资本动向**:资金流入流出追踪 (`fund_flow`)、分红配送历史查询 (`dividend`)。
* **治理结构**:大股东持股变化与股东人数统计 (`holders`)。
...
```bash
# 基本信息查询
PYTHONPATH=/mnt/github/stock/stockAnalyse/src uv run scripts/main.py --action=info --market=SH --symbol=601318

# 历史行情分析 (30 天)
PYTHONPATH=/mnt/github/stock/stockAnalyse/src uv run scripts/main.py --action=history --market=SH --symbol=601318 --start_date=20240201

# 实时盘口数据 (沪深两市全景)
PYTHONPATH=/mnt/github/stock/stockAnalyse/src uv run scripts/main.py --action=spot --market=SH

# 财务报表分析
PYTHONPATH=/mnt/github/stock/stockAnalyse/src uv run scripts/main.py --action=report --market=SH --symbol=601318

# 财务指标分析
PYTHONPATH=/mnt/github/stock/stockAnalyse/src uv run scripts/main.py --action=financial --market=SH --symbol=601318

# 分红查询 (特定日期)
PYTHONPATH=/mnt/github/stock/stockAnalyse/src uv run scripts/main.py --action=dividend --market=SH --date=20240331

# 资金流向追踪
PYTHONPATH=/mnt/github/stock/stockAnalyse/src uv run scripts/main.py --action=fund_flow --market=SH --symbol=601318

# 股东持仓分析
PYTHONPATH=/mnt/github/stock/stockAnalyse/src uv run scripts/main.py --action=holders --market=SH --symbol=601318
```
