...
# 调试：打印可能的模块路径
possible_path = os.path.join(os.path.dirname(__file__), '../../', 'src', 'stock_analyse', 'infrastructure', 'services', 'company_data_service.py')
print(f"检查模块是否存在: {os.path.exists(possible_path)}")

# 调试：打印所有可能的 stock_analyse 路径
for path in sys.path:
    lib_path = os.path.join(path, 'stock_analyse', 'infrastructure', 'services', 'company_data_service.py')
