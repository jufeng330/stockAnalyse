...
def retry_missing_financials():
    ...
    with engine.connect() as con:
        ...
        # ensure table exists by letting it fetch 1 batch if entirely missing or create it via pandas logic
        # wait it was created in create_border_tables.py
    ...
    missing_codes = [c for c in missing_codes if c != 'US.ALTS']
    
    all_data = []
    # Drop batch size to 50 to drastically reduce crash probability and rate limit triggering
    batch_size = 50
    for i in range(0, len(missing_codes), batch_size):
        ...
        while retry < 5:
            try:
                ...
                time.sleep(5)
                retry += 1
        time.sleep(1.5) # Extremely fast per chunk to bypass rate limit
        
        # Save every 500 items (10 chunks of 50)
        if len(all_data) >= 10:
...
def _save_batch(full_df, provider):
    ...
    mapped_df = mapped_df[mapped_df['code'] != '0None']
