...
def retry_missing_financials(market, table_name):
    ...
    # We remove ALTS manually as it breaks Futu SDK snapshot
    missing_codes = [c for c in missing_codes if c != 'US.ALTS']
    
    all_data = []
    # Drop batch size to 100 to reduce crash probability per batch and respect rate limits better
    batch_size = 100
    ...
    if all_data:
        ...
        print(f"No {market} financial data retrieved in retry.")

retry_missing_financials('usa', 'financial_indicate_usa')
