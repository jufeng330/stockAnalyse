...
# 添加项目根目录到路径
sys.path.insert(0, '/home/inspur/codes/stockAnalyse/src')
...
def get_stock_history(market: str, symbol: str, start_date: str = None, end_date: str = None) -> dict:
    ...
    try:
        ...
        if not start_date:
            ...
            start = dt_module.datetime.strptime(end_date, "%Y%m%d") - dt_module.timedelta(days=365)
...
def main():
    ...
    parser.add_argument('--start_date', type=str, help='开始日期 (YYYMMDD)')
    parser.add_argument('--end_date', type=str, help='结束日期 (YYYMMDD)')
