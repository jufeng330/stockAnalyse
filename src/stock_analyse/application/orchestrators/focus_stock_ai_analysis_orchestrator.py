...
from stock_analyse.application.graphs.trading_decision.focus_stock_analysis_graph import run_focus_stock_analysis_graph
from stock_analyse.application.services.ai_stock_data_facade import AIStockDataFacade


class FocusStockAIAnalysisOrchestrator:
    """关注股票普通分析编排器。

    用于 Focus 链路下的 AI 个股分析场景，负责驱动普通股票分析 graph 并归一化最终决策结构。
    """
    ...
    def run(
        ...
        *,
        ...
        progress(5, f'🚀 开始AI个股分析: {stock_code}')
        state = run_focus_stock_analysis_graph(
            callbacks=callbacks,
