...
from stock_analyse.application.graphs.trading_decision.holding_reanalysis_graph import run_holding_reanalysis_graph
from stock_analyse.application.services.ai_stock_data_facade import AIStockDataFacade


class HoldingStockReanalysisOrchestrator:
    """持仓二次分析编排器。

    用于 Holding 链路下的持仓二次分析场景，负责驱动持仓复核 graph 并归一化最终决策结构。
    """
    ...
    def run(
        ...
        *,
        ...
        progress(5, f'🚀 开始持仓二次分析: {stock_code}')
        state = run_holding_reanalysis_graph(
            callbacks=callbacks,
        ...
        progress(95, f'🚀 持仓二次分析完成: {stock_code}')
