...
def execute(market: str, symbol: str, start_year: str | None = None, gateway: stockCompanyInfo | None = None) -> dict:
    try:
        gateway = gateway or stockCompanyInfo(marker=market, symbol=symbol)
        if not start_year:
            start_year = str(datetime.now().year - 5)
        df = gateway.get_stock_financial_analysis_indicator(start_year=start_year)
