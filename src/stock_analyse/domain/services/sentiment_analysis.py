...
# ... 获取数据的逻辑 ...
        ...
        try:
            ...
            try:
                ...
                announcements = ak.stock_notice_report(symbol='全部', date=notice_date)
                if not announcements.empty:
                    processed_announcements = []
                    for _, row in announcements[announcements['代码'].astype(str) == str(stock_code)].head(30).iterrows():
                        announcement = {
                            'title': str(row.get('公告标题', '')),
                            'content': str(row.get('名称', '')),
                            'date': str(row.get('公告日期', datetime.now().strftime('%Y-%m-%d'))),
    ...
    def _get_comprehensive_industry_news(self, stock_code, days=15):
        try:
            stock_name = self.stock_service.get_stock_name()
            ...
            processed_news = []
            keywords = [stock_name, stock_code, '行业', '板块']
            for _, row in news_df.iterrows():
                summary = str(row.get('summary', ''))
                if not any(keyword and keyword in summary for keyword in keywords):
                    continue
                processed_news.append(
                    {
                        'title': str(row.get('tag', '行业新闻')),
                        'content': summary,
            ...
            self.logger.warning(f"获取行业新闻数据源失败: {e}")
            return []
