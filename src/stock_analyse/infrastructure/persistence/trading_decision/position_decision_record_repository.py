...
class PositionDecisionRecordRepository:
    ...
    def create(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        record = {
            ...
            'decision_type': (payload.get('decision_type') or '').strip(),
            'decision_status': (payload.get('decision_status') or '').strip(),
            'conclusion_summary': (payload.get('conclusion_summary') or '').strip(),
            'trigger_summary': (payload.get('trigger_summary') or '').strip(),
        ...
        with self.connection_factory.connect() as connection:
            connection.execute(
                '''
                INSERT INTO position_decision_records (
                    id, holding_stock_id, watch_stock_id, stock_code, stock_name, market,
                    trade_date, analysis_depth, decision_type, decision_status,
                    conclusion_summary, trigger_summary, reason_summary, execution_summary,
                    risk_summary, confidence, tabs_json, evidence_json, raw_result_json,
    ...
    def get_by_id(self, record_id: str) -> dict[str, Any] | None:
        with self.connection_factory.connect() as connection:
            row = connection.execute(
                'SELECT * FROM position_decision_records WHERE id = ?',
                [record_id],
            ).fetchone()
    ...
    def list_by_holding_stock(self, holding_stock_id: str, limit: int = 10) -> list[dict[str, Any]]:
        with self.connection_factory.connect() as connection:
            rows = connection.execute(
                '''
                SELECT *
                FROM position_decision_records
    ...
    def _new_id(self) -> str:
        return f'PDR-{uuid4().hex[:12].upper()}'
