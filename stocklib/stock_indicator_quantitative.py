...
class stockIndicatorQuantitative:
    ...
    def stock_day_data_code(self,stock_code, market, start_date_str, end_date_str):
        ...
        if market == 'usa':
            try:
                ...
                stock_us_hist_df = self.get_stock_sina_daily(stock_code=baba_code, market="usa",
    ...
    """
       ...
       """
    def plot_strategy_mac(self, data, window=20):
    ...
    # 绘制均线策略和布林带策略的图像
    def plot_strategy_bollinger(self, data):
        if data is None:
            print("数据为空，无法绘制图像。")
            return
        ...
        # data['Middle_Band'] = data['Upper_Band'].fillna(method='bfill')
        # data['Lower_Band'] = data['Lower_Band'].fillna(method='bfill')
        # data['Upper_Band'] = data['Upper_Band'].fillna(method='bfill')
        # data['Lower_Band'] = data['Lower_Band'].fillna(method='bfill')
    ...
    # 绘制动量策略的图像
    def plot_strategy_macd(self,data):
        if data is None or data.empty:
            print("数据为空，无法绘制图像。")
            return
    ...
    # 绘制突破策略的图像
    def plot_strategy_breakout(self,data, window=20):
        """
        绘制突破策略的图像
        ...
        data['Signal2'][window:] = np.where(data['收盘'][window:] > data['Previous_High_20'].shift(1)[window:], 1,
                                            np.where(data['收盘'][window:] < data['Previous_Low_20'].shift(1)[window:], -1,
                                                     0))
    ...
    # SAR 策略
    def plot_strategy_sar(self,data):
        """
        绘制 SAR 策略的图像
    ...
    # 均值回归策略
    def plot_mean_reversion_strategy(self,data, window=20, z_score_threshold=1):
        """
        绘制均值回归策略的图像
        :param data: 包含股票价格数据的 DataFrame，需有 '收盘' 列
    ...
    # 次级结构套利
    def plot_sub_structure_arbitrage(self,data, short_window=5, long_window=20, deviation_threshold=0.05):
        """
        绘制次级结构套利策略图像
    ...
    def strategy_mlp_regression(self,data, train_ratio=0.8, epochs=50, batch_size=32):
        ...
        try:
            ...
            buy_indices = data[data['Signal'] == 1].index
            ax1.plot(buy_indices, data['Predicted'][buy_indices], '^', markersize=4, color='g', lw=0,
                     label='Predicted correct')
            sell_indices = data[data['Signal'] == -1].index
            ax1.plot(sell_indices, data['Predicted'][sell_indices], 'v', markersize=4, color='r', lw=0,
                     label='Predicted wrong')
            ax1.legend()
