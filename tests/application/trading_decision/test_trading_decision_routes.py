...
class TestTradingDecisionRoutes:
    ...
    def test_trade_plan_analysis_record_api_persists_and_lists_history(self, tmp_path, monkeypatch):
        ...
        content = response.get_data(as_text=True)
        assert 'positionDecisionPrefill' in content
        assert '继续观察，等待更清晰的催化与估值错配。' in content
        assert 'recommended_action' in content
        assert 'decision_status' in content
        assert 'confidence' in content
        assert '股票分析师' in content

        assert saved['tabs_json'][0]['title'] == detail['tabs_json'][0]['title'] == repo_record['tabs_json'][0]['title']
        assert saved['tabs_json'][4]['title'] == detail['tabs_json'][4]['title'] == repo_record['tabs_json'][4]['title']
        assert saved['tabs_json'][0]['summary'] == detail['tabs_json'][0]['summary'] == repo_record['tabs_json'][0]['summary']
        assert saved['tabs_json'][4]['summary'] == detail['tabs_json'][4]['summary'] == repo_record['tabs_json'][4]['summary']

        assert saved['evidence_json'][0]['detail'] == detail['evidence_json'][0]['detail'] == repo_record['evidence_json'][0]['detail']
        assert saved['evidence_json'][-1]['detail'] == detail['evidence_json'][-1]['detail'] == repo_record['evidence_json'][-1]['detail']

        assert 'positionDecisionPrefill' in response.get_data(as_text=True)
        assert '查看原始 JSON' in response.get_data(as_text=True)
        assert '推荐动作：继续观察' in response.get_data(as_text=True)
        assert '结论摘要：继续观察，等待更清晰的催化与估值错配。' in response.get_data(as_text=True)
        assert 'data-page="position-decision"' in response.get_data(as_text=True)
        assert '/api/trading-decision/position-decision-records' in response.get_data(as_text=True)
        assert '/position-decisions/run' in response.get_data(as_text=True)
        assert '股票分析师' in response.get_data(as_text=True)
        assert '继续观察。' in response.get_data(as_text=True)
        assert '当前仍以跟踪为主' in response.get_data(as_text=True)
        assert '需要新的基本面或价格触发' in response.get_data(as_text=True)

        assert service.get_holding_stock(holding_stock['id'])['suggested_action'] == '继续观察'
        assert service.get_holding_stock(holding_stock['id'])['last_review_at'] == '2026-04-29'

        assert service.position_decision_record_repository.list_by_holding_stock(holding_stock['id'])[0]['id'] == saved['id']
        assert service.position_decision_record_repository.list_by_holding_stock(holding_stock['id'])[0]['decision_type'] == 'watch'
        assert service.position_decision_record_repository.list_by_holding_stock(holding_stock['id'])[0]['decision_status'] == 'observe'
        assert service.position_decision_record_repository.list_by_holding_stock(holding_stock['id'])[0]['conclusion_summary'] == '继续观察，等待更清晰的催化与估值错配。'
        assert service.position_decision_record_repository.list_by_holding_stock(holding_stock['id'])[0]['tabs_json'][0]['title'] == '结论'
        assert service.position_decision_record_repository.list_by_holding_stock(holding_stock['id'])[0]['tabs_json'][4]['title'] == '触发条件'

        assert service.position_decision_record_repository.get_by_id(saved['id'])['id'] == saved['id']
        assert service.position_decision_record_repository.get_by_id(saved['id'])['decision_type'] == 'watch'
        assert service.position_decision_record_repository.get_by_id(saved['id'])['decision_status'] == 'observe'
        assert service.position_decision_record_repository.get_by_id(saved['id'])['conclusion_summary'] == '继续观察，等待更清晰的催化与估值错配。'
        assert service.position_decision_record_repository.get_by_id(saved['id'])['tabs_json'][0]['title'] == '结论'
        assert service.position_decision_record_repository.get_by_id(saved['id'])['tabs_json'][4]['title'] == '触发条件'

        assert saved['raw_result_json']['data']['evidence'][0]['tab'] == '结论'
        assert saved['raw_result_json']['data']['evidence'][1]['tab'] == '风险分析'
        assert saved['raw_result_json']['data']['evidence'][2]['tab'] == '执行注意事项'
        assert saved['raw_result_json']['data']['evidence'][3]['tab'] == '核心理由'
        assert saved['raw_result_json']['data']['evidence'][4]['tab'] == '触发条件'

        assert repo_record['evidence_json'][0]['tab'] == '结论'
        assert repo_record['evidence_json'][1]['tab'] == '风险分析'
        assert repo_record['evidence_json'][2]['tab'] == '执行注意事项'
        assert repo_record['evidence_json'][3]['tab'] == '核心理由'
        assert repo_record['evidence_json'][4]['tab'] == '触发条件'

        assert detail['evidence_json'][0]['tab'] == '结论'
        assert detail['evidence_json'][1]['tab'] == '风险分析'
        assert detail['evidence_json'][2]['tab'] == '执行注意事项'
        assert detail['evidence_json'][3]['tab'] == '核心理由'
        assert detail['evidence_json'][4]['tab'] == '触发条件'

        assert records[0]['evidence_json'][0]['tab'] == '结论'
        assert records[0]['evidence_json'][1]['tab'] == '风险分析'
        assert records[0]['evidence_json'][2]['tab'] == '执行注意事项'
        assert records[0]['evidence_json'][3]['tab'] == '核心理由'
        assert records[0]['evidence_json'][4]['tab'] == '触发条件'

        assert detail_payload['evidence_json'][0]['tab'] == '结论'
        assert detail_payload['evidence_json'][1]['tab'] == '风险分析'
        assert detail_payload['evidence_json'][2]['tab'] == '执行注意事项'
        assert detail_payload['evidence_json'][3]['tab'] == '核心理由'
        assert detail_payload['evidence_json'][4]['tab'] == '触发条件'

        assert saved['raw_result_json']['data']['tabs'][0]['id'] == 'conclusion'
        assert saved['raw_result_json']['data']['tabs'][1]['id'] == 'risk'
        assert saved['raw_result_json']['data']['tabs'][2]['id'] == 'execution'
        assert saved['raw_result_json']['data']['tabs'][3]['id'] == 'reason'
        assert saved['raw_result_json']['data']['tabs'][4]['id'] == 'trigger'

        assert repo_record['raw_result_json']['data']['tabs'][0]['id'] == 'conclusion'
        assert repo_record['raw_result_json']['data']['tabs'][4]['id'] == 'trigger'
        assert detail['raw_result_json']['data']['tabs'][0]['id'] == 'conclusion'
        assert detail['raw_result_json']['data']['tabs'][4]['id'] == 'trigger'
        assert records[0]['raw_result_json']['data']['tabs'][0]['id'] == 'conclusion'
        assert records[0]['raw_result_json']['data']['tabs'][4]['id'] == 'trigger'
        assert detail_payload['raw_result_json']['data']['tabs'][0]['id'] == 'conclusion'
        assert detail_payload['raw_result_json']['data']['tabs'][4]['id'] == 'trigger'

        assert saved['raw_result_json']['data']['decision']['summary'] == saved['conclusion_summary']
        assert repo_record['raw_result_json']['data']['decision']['summary'] == repo_record['conclusion_summary']
        assert detail['raw_result_json']['data']['decision']['summary'] == detail['conclusion_summary']
        assert detail_payload['raw_result_json']['data']['decision']['summary'] == detail_payload['conclusion_summary']

        assert saved['raw_result_json']['data']['meta']['role'] == '股票分析师'
        assert repo_record['raw_result_json']['data']['meta']['role'] == '股票分析师'
        assert detail['raw_result_json']['data']['meta']['role'] == '股票分析师'
        assert detail_payload['raw_result_json']['data']['meta']['role'] == '股票分析师'

        assert saved['raw_result_json']['data']['context_snapshot']['source'] == 'test'
        assert repo_record['raw_result_json']['data']['context_snapshot']['source'] == 'test'
        assert detail['raw_result_json']['data']['context_snapshot']['source'] == 'test'
        assert detail_payload['raw_result_json']['data']['context_snapshot']['source'] == 'test'

        assert saved['raw_result_json']['data']['analysis_depth'] == 'deep'
        assert repo_record['raw_result_json']['data']['analysis_depth'] == 'deep'
        assert detail['raw_result_json']['data']['analysis_depth'] == 'deep'
        assert detail_payload['raw_result_json']['data']['analysis_depth'] == 'deep'

        assert saved['raw_result_json']['data']['trade_date'] == '2026-04-29'
        assert repo_record['raw_result_json']['data']['trade_date'] == '2026-04-29'
        assert detail['raw_result_json']['data']['trade_date'] == '2026-04-29'
        assert detail_payload['raw_result_json']['data']['trade_date'] == '2026-04-29'

        assert saved['raw_result_json']['data']['holding_stock_id'] == ''
        assert saved['raw_result_json']['data']['watch_stock_id'] == ''
        assert saved['raw_result_json']['data']['stock_code'] == ''
        assert saved['raw_result_json']['data']['stock_name'] == ''
        assert saved['raw_result_json']['data']['market'] == ''

        assert 'position_decision_raw_prefill' != ''
        ...
        records = service.list_position_decision_records(holding_stock['id'])
        assert records
        ...
        detail_payload = detail_response.get_json()['data']
        assert detail_payload['id'] == saved['id']
        ...
        assert saved['decision_type'] == 'watch'
        assert saved['decision_status'] == 'observe'
        assert saved['confidence'] == 'medium'
        assert saved['decision_type_label'] == '继续观察'
        assert saved['conclusion_summary'] == '继续观察，等待更清晰的催化与估值错配。'
        ...
        assert repo_record['decision_status'] == 'observe'
        assert repo_record['confidence'] == 'medium'
        assert repo_record['conclusion_summary'] == '继续观察，等待更清晰的催化与估值错配。'

        assert records[0]['decision_type'] == 'watch'
        assert records[0]['decision_status'] == 'observe'
        assert records[0]['confidence'] == 'medium'
        assert records[0]['decision_type_label'] == '继续观察'
        assert records[0]['conclusion_summary'] == '继续观察，等待更清晰的催化与估值错配。'

        assert detail_payload['decision_type'] == 'watch'
        assert detail_payload['decision_status'] == 'observe'
        assert detail_payload['confidence'] == 'medium'
        assert detail_payload['decision_type_label'] == '继续观察'
        assert detail_payload['conclusion_summary'] == '继续观察，等待更清晰的催化与估值错配。'

        assert saved['tabs_json']
        assert detail['tabs_json']
        assert repo_record['tabs_json']
        assert records[0]['tabs_json']
        assert detail_payload['tabs_json']

        assert saved['evidence_json']
        assert detail['evidence_json']
        assert repo_record['evidence_json']
        assert records[0]['evidence_json']
        assert detail_payload['evidence_json']

        assert saved['raw_result_json']
        assert detail['raw_result_json']
        assert repo_record['raw_result_json']
        assert records[0]['raw_result_json']
        assert detail_payload['raw_result_json']

        assert saved['raw_result_json']['success'] is True
        assert detail['raw_result_json']['success'] is True
        assert repo_record['raw_result_json']['success'] is True
        assert records[0]['raw_result_json']['success'] is True
        assert detail_payload['raw_result_json']['success'] is True
        ...
        assert '核心理由' in content
        assert '执行注意事项' in content
        assert '风险分析' in content
        assert '结论' in content
        assert '买卖决策结果' in content
        assert '查看原始 JSON' in content
        assert '推荐动作：继续观察' in content
        assert '结论摘要：继续观察，等待更清晰的催化与估值错配。' in content
        assert '当前仍以跟踪为主' in content
        assert '需要新的基本面或价格触发' in content
        assert 'data-page="position-decision"' in content
        assert '/api/trading-decision/position-decision-records' in content
        assert '/position-decisions/run' in content
        assert 'position_decision' in content
        assert 'recommended_action' in content
        assert 'decision_status' in content
        assert 'confidence' in content
        assert 'conclusion_summary' in content
        ...
        assert saved['raw_result_json']['data']['decision']['action'] == 'watch'
        assert saved['raw_result_json']['data']['decision']['status'] == 'observe'
        assert saved['raw_result_json']['data']['decision']['confidence'] == 'medium'
        assert saved['raw_result_json']['data']['decision']['summary'] == '继续观察，等待更清晰的催化与估值错配。'
        assert saved['raw_result_json']['data']['meta']['role'] == '股票分析师'
        assert saved['raw_result_json']['data']['context_snapshot']['source'] == 'test'
        assert saved['raw_result_json']['data']['tabs'][0]['title'] == '结论'
        assert saved['raw_result_json']['data']['tabs'][4]['title'] == '触发条件'
        assert saved['raw_result_json']['data']['evidence'][0]['tab'] == '结论'
        assert saved['raw_result_json']['data']['evidence'][-1]['tab'] == '触发条件'

        assert holding_detail['suggested_action'] == '继续观察'
        assert holding_detail['last_review_at'] == '2026-04-29'

        assert list_response.get_json()['data'][0]['id'] == saved['id']
        assert detail_response.get_json()['data']['id'] == saved['id']

        assert service.get_position_decision_record(saved['id'])['id'] == saved['id']
        assert service.list_position_decision_records(holding_stock['id'])[0]['id'] == saved['id']
        assert service.position_decision_record_repository.get_by_id(saved['id'])['id'] == saved['id']
        ...
        assert '结论摘要：继续观察，等待更清晰的催化与估值错配。' in response.get_data(as_text=True)
        assert '查看原始 JSON' in response.get_data(as_text=True)
        assert '买卖决策结果' in response.get_data(as_text=True)
        assert 'data-page="position-decision"' in response.get_data(as_text=True)
        assert '触发条件' in response.get_data(as_text=True)
        assert '核心理由' in response.get_data(as_text=True)
        assert '执行注意事项' in response.get_data(as_text=True)
        assert '风险分析' in response.get_data(as_text=True)
        assert '结论' in response.get_data(as_text=True)

        assert saved['id']
        assert saved['decision_type'] == 'watch'
        assert saved['decision_status'] == 'observe'
        assert saved['conclusion_summary'] == '继续观察，等待更清晰的催化与估值错配。'
        assert saved['tabs_json'][0]['title'] == '结论'
        assert saved['tabs_json'][4]['title'] == '触发条件'
        assert saved['evidence_json'][0]['detail'] == '当前仍以跟踪为主'
        assert saved['evidence_json'][-1]['detail'] == '需要新的基本面或价格触发'
        ...
        assert repo_record['raw_result_json']['data']['decision']['action'] == 'watch'
        assert repo_record['tabs_json'][0]['title'] == '结论'
        assert repo_record['tabs_json'][4]['title'] == '触发条件'

        assert records[0]['raw_result_json']['data']['decision']['action'] == 'watch'
        assert detail_payload['raw_result_json']['data']['decision']['action'] == 'watch'
        ...
        content = response.data.decode('utf-8')
        assert saved['id'] in content
        assert '已加载历史买卖决策记录' in content
        assert 'renderPositionDecisionResult' in content
        assert '继续观察' in content
        assert f'/position-decision?holding_stock_id={holding_stock["id"]}&record_id={saved["id"]}' in content
        assert '查看原始 JSON' in content
        assert '决策类型' not in content
